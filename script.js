...
const villageUrl = 'https://raw.githack.com/GardenXas/ai-rpg-2d/main/locations/village.js';

let villageStructure;
            ...
            for (let x = 0; x < structure.width; x++) {
                if (startY + y < mapHeight && startX + x < mapWidth) {
                    if (structure.map[y][x] !== ' ') {
                        map[startY + y] = map[startY + y].substring(0, startX + x) + structure.map[y][x] + map[startY + y].substring(startX + x + 1);
                    }
                }
            }
        }
        ...
        while (currentX !== endX || currentY !== endY) {
            ...
            if (currentY < mapHeight && currentX < mapWidth && currentY >= 0 && currentX >= 0) {
                if (map[currentY][currentX] === '.') {
                    map[currentY] = map[currentY].substring(0, currentX) + '=' + map[currentY].substring(currentX + 1);
                }
            }
        }
    }
    ...
    function renderMap() {
        ...
        for (let y = startY; y < endY; y++) {
            for (let x = startX; x < endX; x++) {
                ...
                if (entity) {
                    entityString += `<span style="position: absolute; left: ${x - startX}em; top: ${y - startY}em;">${entity}</span>`;
                } else if (cell === 'T') {
                    objectString += `<span class="tree" style="position: absolute; left: ${x - startX}em; top: ${y - startY}em;">T</span>`;
                } else if (cell === 'O') {
                    objectString += `<span class="rock" style="position: absolute; left: ${x - startX}em; top: ${y - startY}em;">O</span>`;
                } else if (cell === '^') {
                    objectString += `<span class="mountain" style="position: absolute; left: ${x - startX}em; top: ${y - startY}em;">^</span>`;
                } else if (cell === '=') {
                    objectString += `<span class="road" style="position: absolute; left: ${x - startX}em; top: ${y - startY}em;">=</span>`;
                } else {
                    groundString += `<span style="position: absolute; left: ${x - startX}em; top: ${y - startY}em;">${cell}</span>`;
                }
            }
        }
